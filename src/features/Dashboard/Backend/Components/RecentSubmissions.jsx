import React from "react";
import { FileText, Info, Loader } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import CustomButton from "@/components/common/Buttons/CustomButton";
import { Tooltip } from "@/components/common/Tooltip";
import { Clock, Loader2, CheckCircle, AlertTriangle } from "lucide-react";

const statusConfig = {
  PENDING: {
    label: "Pending",
    style: "bg-gray-100 text-gray-600",
    icon: <Clock size={12} />,
  },
  IN_PROGRESS: {
    label: "In Progress",
    style: "bg-blue-100 text-blue-600",
    icon: <Loader size={12} className="animate-spin" />,
  },
  COMPLETED: {
    label: "Completed",
    style: "bg-green-100 text-green-600",
    icon: <CheckCircle size={12} />,
  },
  VERIFIED: {
    label: "Verified",
    style: "bg-green-100 text-green-600",
    icon: <CheckCircle size={12} />,
  },
  FAILED: {
    label: "Failed",
    style: "bg-red-100 text-red-600",
    icon: <AlertTriangle size={12} />,
  },
};

const RecentSubmissions = ({ data = [] }) => {
  const navigate = useNavigate();
  const safeData = Array.isArray(data) ? data : [];

  return (
    <div className="bg-white rounded-xl shadow-sm border p-4 w-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm font-semibold text-textprimary">
          RECENT SUBMISSIONS
        </h2>

        <button className="text-xs text-primary font-medium hover:underline">
          <Link to="/verification-cases">View All →</Link>
        </button>
      </div>

      <div className="space-y-4">
        {safeData.map((item, index) => (
          <div
            key={item?._id || item?.caseId || index}
            className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-gray-100">
                <FileText size={18} className="text-orange-500" />
              </div>

              <div>
                <p className="text-sm font-semibold text-textprimary">
                  {item?.subjectName || "-"}
                </p>
                <p className="text-xs text-textsecondary">
                  {item?.caseId || "-"}
                </p>
              </div>
            </div>

            <div className="flex items-center gap-3">
              {/* <div className="flex flex-col items-end gap-1"> */}
              <span
                className={`flex items-center gap-1 text-[10px] px-3 py-1 rounded-full font-semibold ${statusConfig[item?.status]?.style ||
                  "bg-gray-100 text-gray-500"
                  }`}
              >
                {statusConfig[item?.status]?.icon}
                {statusConfig[item?.status]?.label || item?.status || "Unknown"}
              </span>
              {!item?.isFormCompleted ? (<span
                className={`flex items-center gap-1 text-[10px] px-3 py-1 border border-red-600 text-red-600 bg-red-100 rounded-full font-semibold
                `}
              >
                <Info size={12} />
                Your form is not complete yet.
              </span>) :
                (<span
                  className={`flex items-center gap-1 text-[10px] px-3 py-1  border-gray-600 text-gray-600  font-semibold
                `}
                >

                  Submitted on : {item?.createdAt ? new Date(item.createdAt)?.toLocaleDateString() : "No date"}{item?.createdAt ? new Date(item.createdAt)?.toLocaleTimeString() : ""}
                </span>)}
              {/* </div> */}
              <Tooltip title="View Details">
                <CustomButton
                  className="text-xs"
                  onClick={() => navigate(`/review-confirm/${item?._id}`)}
                  disabled={!item?._id}
                >
                  View
                </CustomButton>
              </Tooltip>
            </div>
          </div>
        ))}

        {!safeData.length && (
          <p className="text-sm text-gray-500">No recent submissions found.</p>
        )}
      </div>
    </div>
  );
};

export default RecentSubmissions;
