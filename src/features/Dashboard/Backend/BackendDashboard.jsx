...
import { BackendDashboardDataList } from "@/Services/backendDashboardServices";
...
const BackendDashboard = () => {
  ...
  const fetchDashData = async () => {
    try {
      setLoading(true);
      const res = await BackendDashboardDataList();
      if (res.status === 200) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        {cards.map((card, index) => {
          let value = 0;
          if (card.key === "assigned") {
            value = data?.stats?.assigned || data?.stats?.inProgress || 0;
          } else if (card.key === "pending") {
            value = data?.stats?.pending || data?.stats?.pendingPayment || 0;
          } else {
      ...
      <div className="flex p-2">
        <RecentSubmissions data={data?.recentTasks} />
      </div>
    </>
  );
};
